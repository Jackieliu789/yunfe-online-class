...
<div id="app" v-cloak>
    <div class="app-container">
        <div class="layui-row layui-col-space20" style="background:#F2F2F2;">
            ...
            <div class="layui-col-md6 layui-col-xs6 layui-col-sm6">
                <div class="layui-card">
                    <div class="layui-card-header">已选嘉宾</div>
                    <div class="layui-card-body step-list">
                        ...
                        <div class="layui-row step-item" v-if="users.length <= 0">
                            <div class="layui-col-md12">暂无嘉宾</div>
                        </div>
                    </div>
                </div>
                <#--end card-->
    ...
    var app = new Vue({
        ...
        methods: {
            ...
            treeClick: function (event, treeId, treeNode, clickFlag) {
                ...
                var that = this;
                $.http.post("${params.contextPath}/web/pmsLiveCourse/saveUser.json", {
                    liveCourseId: this.courseId,
                    userId: userId
                }).then(function (data) {
                    if (!data.success) {
                        $.message(data.message);
            ...
            loadUsers: function () {
                var that = this;
                $.http.post("${params.contextPath}/web/pmsLiveCourse/queryUsers.json", {liveCourseId: this.courseId}).then(function (data) {
                    if (!data.success) {
                        $.message(data.message);
                        return;
                    }
            ...
            removeUser: function (index) {
                ...
                var that = this;
                $.http.post("${params.contextPath}/web/pmsLiveCourse/removeUser.json", {id: row.id}).then(function (data) {
                    $.message(data.message);
                    if (!data.success) {
                        return;
                    }

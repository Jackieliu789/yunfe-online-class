    ...
    <div class="ui-form">
        ...
        <#if user>
            <form class="layui-form" @submit.prevent="submitForm()" method="post">
                <h1 class="text-center" style="font-size:28px;margin:50px 0;">互动课程邀请码验证</h1>
                <div class="layui-form-item">
...
<script>
    var app = new Vue({
        ...
        methods: {
            <#if user>
            submitForm: function () {
                $.http.post("${params.contextPath}/web/pmsCourseItem/check/invit/code.json", this.record).then(function (data) {
                    if (!data.success) {
            ...
            loginForm: function () {
                $.http.post("${params.contextPath}/loginIn.json", this.login).then(function (data) {
                    ...
                    location.href = "${params.contextPath!}/course/share/${params.cid!}.json";
                });
            }
        }
    });
